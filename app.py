...
def home():
    ...
    # Fetch user details from the database if logged in
    profile_image_id = None
    if is_logged_in:
        user = mongo.db.users.find_one({'_id': ObjectId(session['user_id'])})
        if user:
            user_name = user.get('name', user_name)  # Update name from database if available
...
def profile():
    ...
    if request.method == 'POST':
        ...
        if profile_image and profile_image.filename != '':
            image_id = fs.put(profile_image.read(), filename=profile_image.filename, content_type=profile_image.content_type)
            update_data['profileImageId'] = image_id
        ...
        try:
            msg = Message(
                subject="Smart Plant Care Profile Updated",
